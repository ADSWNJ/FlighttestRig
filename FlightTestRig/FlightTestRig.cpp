...
FlightTestRig_GCore *g_SC = nullptr;    // points to the static persistence core
char *g_moduleName = "FlightTestRig";
...
FlightTestRig::FlightTestRig (DWORD w, DWORD h, VESSEL *vessel, UINT mfd)
: MFD2 (w, h, vessel)
{
  if (g_SC == nullptr) {
    g_SC = new FlightTestRig_GCore();
    GC = g_SC;
  ...
  VC = (FlightTestRig_VCore*) GC->P.findVC(vessel);		  // Locate our vessel core
  if (!VC) {
    VC = new FlightTestRig_VCore(vessel, GC);				    // ... if missing, initialize it.
    GC->P.addVC(vessel, VC);
  }

  LC = (FlightTestRig_LCore*) GC->P.findLC(vessel, mfd);	// Locate our local (vessl+MFD position) core
  if (!LC) {
    LC = new FlightTestRig_LCore(vessel, mfd, GC);			  // ... if missing, initialize it.
...
void FlightTestRig::ReadStatus(FILEHANDLE scn) {
...
void FlightTestRig::WriteStatus(FILEHANDLE scn) const {
