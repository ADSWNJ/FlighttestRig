// ==========================================================================
//
//	FlightTestRig (Sketchpad Helper Functions for MFD Display Update)
//	=================================================================
//
...
int FlightTestRig::_Line(const int row) {  // row is 0-24, for 24 rows. e.g. Line(12)
  int ret;
  ret = (int)((H - (int)(ch / 4)) * row / 25) + (int)(ch / 4);
  return ret;
};

int FlightTestRig::_Col(const int pos) {  // pos is 0-5, for 6 columns. Eg Col(3) for middle
  int ret = (int)((W - (int)(cw / 2)) * pos / 6) + int(cw / 2);
  return ret;
};

int FlightTestRig::_Col2(const int pos) {  // pos is 0-11, for 12 columns. Eg Col(6) for middle
...
void FlightTestRig::skpFormatText(const int col, const int line, const char* fmt, ...) {
...
void FlightTestRig::skpFmtColText(const int col, const int line, const bool test, const DWORD truecol, const DWORD falsecol, const char* fmt, ...) {
...
void FlightTestRig::skpColor(const DWORD col) {
  LC->skp->SetTextColor(col);
}

void FlightTestRig::skpFmtEngText(const int col, const int line, const char* fmt, const char* sfx, const double val, const int dloB) {
  LC->skpColPix = _Col(col);
  LC->skpLinePix = _Line(line);
...
void FlightTestRig::skpFmtEngText(const int col, const int line, const char* fmt, const unsigned char* sfx, const double val, const int dloB) {
  LC->skpColPix = _Col(col);
  LC->skpLinePix = _Line(line);
  char engUnit[12] = "pnum kMGTPE";
  double cnvVal = val;
...
void FlightTestRig::skpTitle(const char *title) {
  Title(LC->skp, title);
...
bool FlightTestRig::DisplayMessageMode() {
...
void FlightTestRig::showMessage() {
