...
#include "FlightTestRig_Interface.hpp"

// ====================================================================================================================
// Global variables

FlightTestRig_Interface *g_OrbIF = nullptr;
extern FlightTestRig_GCore *g_SC;    // points to the static persistence core
...
{
  g_OrbIF = new FlightTestRig_Interface(hDLL);
  g_SC = nullptr;
  MFDMODESPECEX spec;
  spec.name = g_moduleName;
  spec.key = OAPI_KEY_S;
  spec.context = NULL;
  spec.msgproc = FlightTestRig_Interface::MsgProc;
 ...
 */
int FlightTestRig_Interface::MsgProc (UINT msg, UINT mfd, WPARAM wparam, LPARAM lparam)
{
  switch (msg) {
  case OAPI_MSG_MFD_OPENED:
    return (int)(new FlightTestRig (LOWORD(wparam), HIWORD(wparam), (VESSEL*)lparam, mfd));
  }
  return 0;
}

FlightTestRig_Interface::FlightTestRig_Interface(HINSTANCE hDLL) : oapi::Module( hDLL ) {}
FlightTestRig_Interface::~FlightTestRig_Interface() {}

/*
 * clbkSimulationStart called when Orbiter launches
 */
void FlightTestRig_Interface::clbkSimulationStart(RenderMode mode) {
  char buf[128];
 ...
 */
void FlightTestRig_Interface::clbkSimulationEnd() {                                      // When we exit sim back to Launchpad, make sure we tidy things up properly
  if (g_SC != nullptr) {
 ...
 */
void FlightTestRig_Interface::clbkPreStep(double simt, double simdt, double mjd) {      // Called on each iteration of the calc engine (more often than the MFD Update
 ...
 */
//void FlightTestRig_Interface::clbkPostStep(double simt, double simdt, double mjd) {}

/*
 * clbkDeleteVessel called when a vessel is deleted
 */
void FlightTestRig_Interface::clbkDeleteVessel(OBJHANDLE hVessel) {                     // Tidy up when a vessel is deleted (stops clbkPreStep calling a dead vessel)
