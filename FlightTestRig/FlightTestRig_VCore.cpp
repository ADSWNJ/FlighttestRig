// ==============================================================
//
//	FlightTestRig (Vessel Core Persistence)
//	=======================================
//
...
FlightTestRig_VCore::~FlightTestRig_VCore() {
...
void FlightTestRig_VCore::dumpMW() {
  FILE *hFile = nullptr;
  errno_t err = fopen_s(&hFile, ".\\Config\\MFD\\FlightTestRig\\FlightTestRig_MW.csv", "w");
...
void FlightTestRig_VCore::logOpen() {
  logState = 0;
  errno_t err = fopen_s(&hLogFile, ".\\Config\\MFD\\FlightTestRig\\FlightTestRig_Dump.csv", "w");
  if (err != 0 || hLogFile == nullptr) {
    logError(err, "trying to open .\\Config\\MFD\\FlightTestRig\\FlightTestRig_Dump.csv at MJD %.5f", mjd);
    hLogFile = nullptr;
    return;
  }
...
void FlightTestRig_VCore::logWrite() {
  int bytesWritten;
...
void FlightTestRig_VCore::logClose() {
  fclose(hLogFile);
  hLogFile = nullptr;
  logState = 0;
  return;
...
void FlightTestRig_VCore::logError(errno_t err, const char *fmt...) {
  char buf1[256];
  char fmtBuf[256];
  char buf2[256];
...
void FlightTestRig_VCore::logError(const char *fmt...) {
  char buf1[256];
  char fmtBuf[256];
  char buf2[256];
...
//
double FlightTestRig_VCore::Macvicar_Whelan_Err_Function(double E) {
...
double FlightTestRig_VCore::Macvicar_Whelan_dErr_Function(double dE) {
...
double FlightTestRig_VCore::Macvicar_Whelan_Fuzzy_Function(double E, double dE) {
...
bool FlightTestRig_VCore::GetVesselClassControlSettings() {
  ...
  const char VesselParamsFile[] = ".\\Config\\MFD\\FlightTestRig\\FlightTestRig_VesselParams.cfg";
